...
图结构就是 [多叉树结构](19-binary-tree-traversal.md) 的延伸。图结构逻辑上由若干节点（Vertex）和边（Edge）构成，我们一般用邻接表、邻接矩阵等方式来存储图。
...
注意看我把「逻辑结构」和「具体实现」分开了。就好比前文 [二叉堆的原理和实现](./25-binary-heap-principle-visualization.md) 一样，二叉堆的逻辑结构是一棵完全二叉树，但我们实际上用的数组来实现它。
...
看到这个实现，你有没有很熟悉？它和我们之前说的 [多叉树节点](./21-n-ary-tree-traversal.md) 几乎完全一样：
