...
其实你可以很容易地直接从标准的 BFS 遍历出发，推演出 Dijkstra 算法的代码。来思考标准的 [二叉树 BFS（层序）遍历](19-binary-tree-traversal.md) 和 [图的 BFS 遍历](./31-graph-dfs-bfs-traversal.md)，为什么它们能算出最短路径？
...
如何优先扩散权重和最小的路径呢？借助 优先级队列 把权重小的路径排前面，[优先级队列](./26-binary-heap-priority-queue.md)的逻辑是每次出队权重最小的节点，最先到达终点的路径就是权重和最小的路径。
