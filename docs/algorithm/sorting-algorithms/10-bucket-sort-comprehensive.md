...
桶排序算法可能并不常见，但我个人感觉它的思想非常有意思，因为你可以在它的算法思想中同时看到前面讲的 [归并排序](07-merge-sort-postorder.md) 和 [计数排序](09-counting-sort-principle.md) 的影子。
...
合并多个有序链表/数组 的通用算法，但那个算法会用到 [二叉堆结构](../data-structures/25-binary-heap-principle-visualization.md)，且复杂度为 O(n∗logk)，这里显然不适用：
...
桶排序可以自选对每个桶的排序函数，一般的选择是 [插入排序](./04-insertion-sort-reverse-thinking.md)，因为插入排序属于比较简单的稳定排序算法，而且在几个 O(n²) 的基本排序算法中，插入排序的综合能力较强。
